...
troponin <- read.delim("/Cluster_Filespace/Marioni_Group/Ola/GS_added_biomarkers/GS20K_Troponin_all.PHE")
names(troponin)[1] <- "Sample_Name"
bmi = read.csv('/Cluster_Filespace/Marioni_Group/GS/GS_dataset/clinical/body.csv');
names(bmi)[1] <- "Sample_Name"

joined_bmi_troponin = left_join(troponin, bmi, by = "Sample_Name")
d1 = joined_bmi_troponin[c(1:4, 8)] #neglecting troponin I
...
print(summary(lm(W3[,4] ~ W3$age.x + factor(W3$sex.x), data=W3, na.action="na.exclude"))$r.squared)
...
studied_proteins = c("Troponin_T", "bmi")
